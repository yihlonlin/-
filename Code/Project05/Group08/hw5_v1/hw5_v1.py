...
def audioRecorderCallback(fname):
    ...
    try:
        ...
        print(text)
        saysentence("search.mp3")
        input_text = hw1v2.findtext(text)
        try:
            rex = re.sub('[\W]', '', input_text)
            comtext = idcompare.compaere_name(rex)
            if comtext == False:
                saysentence("checkname.mp3")
            else:
                find_data.find_time(comtext)
        except TypeError:
            saysentence("checkname.mp3")
...
def saysentence(inputfil):
    mixer.init()
    mixer.music.load(inputfil)
    mixer.music.play()
